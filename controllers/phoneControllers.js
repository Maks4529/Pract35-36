...
module.exports.getPhoneById = async (req, res, next) => {
    ...
    try {
        ...
        if (!foundPhoneById){
           return res.status(404).send([{status: 404, message: "Phone is not found"}]);
        };
...
module.exports.getPhonesByYear = async (req, res, next) => {
    ...
    try {
        ...
        if (foundPhones.length === 0){
            return res.status(404).send([{status: 404, message: "Phones not found."}]);
        };
...
module.exports.updatePhonesByYear = async (req, res, next) => {
    ...
    try {
        ...
        const preparedPhone = updatedPhones.map(phone => _.omit(phone, ['createdAt', 'updatedAt']));
...
module.exports.deletePhonesByYear = async (req, res, next) => {
    ...
    try {
        const deletedPhonesCount = await Phone.destroy({where: {
            yearOfManufacture: {
                [Op.between]: [start, end],
            },
        }});
